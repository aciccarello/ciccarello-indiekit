...
/**
 * Simplify JF2 wrapper objects
 * @param {unknown} value
 * @returns The object if it has properties or the first child if it is only a wrapper
 */
const normalizeToOneValue = (value) => {
  ...
  if (objectKeys.length === 1 && "children" in value && Array.isArray(value.children)) {
...
export default async function formatFile(properties) {
  try {
    ...
    let references = Object.entries(referencesMap).map(([url, ref]) => {
      ...
      if (!author) {
        // Assume first card card on entry is the author
      ...
      if (author) {
        author = normalizeToOneValue(author);
      } else if (name) {
      ...
      return {
        ...
        "post-type": postType,
        type,
        published: normalizeToOneValue(published),
        updated: normalizeToOneValue(updated),
        author,
    ...
    const syndication = toArray(properties.syndication);
    ...
    properties = {
      ...
      // Usually mp-syndicate-to isn't passed but our wrapper syndicator will add it to the request body as syndication
      syndication: syndication.length > 0 ? syndication : undefined,
    };
